...
export async function findSlipByFingerprint(
  fingerprint: string,
): Promise<{ id: string; ledgerRef: string | null } | null> {
  ...
  if (!data) return null;
  return { id: data.id, ledgerRef: data.ledger_ref ?? null };
}
...
export type MarketSource = 'bitkub' | 'binance_th' | 'manual';

/**
 * เรตที่ใช้คำนวณ:
 * - sellRate       = เรตขายของห้อง (แอดมินตั้งผ่าน /setrate)
 * - marketUsdtRate = Bitkub THB_USDT last, fallback Binance TH USDTTHB
 */
export async function getLatestRates(): Promise<{
  ...
  const result = {
    sellRate: Number.isFinite(sellRate) && sellRate > 0 ? sellRate : 0,
    marketUsdtRate,
    marketSource,
  };
  cachedRates = result;
