...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(sellRate) || sellRate <= 0 || sellRate > 1000) {
    return NextResponse.json(
      { data: null, error: { code: 'INVALID_RATE', message: 'sellRate ต้องเป็นตัวเลข 0-1000' } },
      { status: 400 }
    );
  }
  if (!Number.isFinite(marketRate) || marketRate <= 0 || marketRate > 1000) {
    return NextResponse.json(
      { data: null, error: { code: 'INVALID_RATE', message: 'marketRate ต้องเป็นตัวเลข 0-1000' } },
      { status: 400 }
    );
  }
