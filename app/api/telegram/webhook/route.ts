...
async function handleUpdate(update: any): Promise<void> {
  ...
  if (admin && text) {
    const handled = await handleCtText({ chatId, userId, admin, text });
    if (handled) return;
  }
  ...
  if (cmd === 'pin') {
    ...
    if (!rest) {
      ...
      await sendMessage(chatId, C.pinView(pinned.map((bank) => ({
        bank: bank.bank_name,
        last4: accountLast4(bank.account_number) ?? '????',
      }))));
      return;
    }
    const { parseDeskPin } = await import('@/bot/parse');
    const desk = parseDeskPin(rest);
    const bankInput = desk?.bank ?? rest.split(/\s+/)[0];
    const accountInput = desk?.account ?? rest.split(/\s+/).slice(1).join('');
    if (!bankInput || String(accountInput).replace(/\D/g, '').length < 4) {
      await sendMessage(chatId, UI.error('ส่งข้อความ pin ทั้งก้อน หรือ /pin BBL 0989887823'));
      return;
    }
    try {
      const result = await pinBankAccount(chatId, bankInput, accountInput);
      await sendMessage(chatId, C.pinView(result.pinned.map((bank) => ({
        bank: bank.bank_name,
        last4: accountLast4(bank.account_number) ?? '????',
      }))));
    } catch (error: any) {
